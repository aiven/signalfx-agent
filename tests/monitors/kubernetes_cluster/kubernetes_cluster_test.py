...
from tests.helpers.util import ensure_always, get_monitor_metrics_from_selfdescribe, wait_for, wait_for_assertion
...
def test_node_metrics_and_props(k8s_cluster):
    ...
    with k8s_cluster.run_agent(agent_yaml=config) as agent:
        for node in k8s_cluster.client.CoreV1Api().list_node().items:
            ...
            expected_props = {"kubernetes_node_label_" + k: v for k, v in node.metadata.labels.items()}
